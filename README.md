...
useMemo와 useCallback
    메모이제이션, 랜더링 최적화
        ...
        - 어떤 함수가 생성되거나 해당 값이 연산되는 결과가 오래걸린다면
        - 리엑트가 리랜더링할 때마다 해당 함수를 다시 초기화 후 재선언 해야할 필요가 있는가
        - 물론, 내부 로직이(스코프에 참조된 값이) 수정된다면 다시 호출 해주어야하지만
        - 값이 바뀌지 않았다면 이전 스테이트와 같은 똑같은 함수를 재선언 할 필요가 있는가

        - 당연히 없습니다.
        - 따라서 리엑트에서는 메모이제이션을 활용하여 재선언 할 필요가 없는 함수를 랜더링이
        되더라도 기존의 값을 유지하면서 필요없는 랜더링 시간을 최적화

        - React.memo, useMemo, useCallback, useRef 등이 있습니다
        - * 그렇다면 모든 함수를 메모이제이션 해야하는가?
        - 최적화에는 비용이 필요합니다. 따라서 최적화가 필요없는 간단한 로직에는 메모이제이션 X

    * useMemo
        연산된 값을 반환하여 랜더링 되었을 때 재사용이 가능 하도록 해주는 훅 함수
...
추가
   ...
   BoardDTO안에 FileVO List가 있음!
   화면으로부터 한 번에 FileVO까지 전달받는다.
   서비스에 BoardDTO 전달!
   서비스에서는 게시글을 먼저 INSERT해주고
   첨부파일에 게시글 번호를 등록하여 첨부파일을 INSERT해준다.
   
조회
   - 게시글 조회(BoardNumber)
   - 첨부파일 조회(BoardNumber)

   서비스에서 첨부파일에서 파일의 크기를 KB로 변경한 뒤
   BoardDTO에 그대로 첨부파일 목록을 추가한다.


수정
   - 기존 첨부파일 조회
   - 서버에서(C:/upload) 기존 첨부파일 삭제
   - 기존 첨부파일 삭제(DB에서)
   - 새로운 첨부파일 추가
   - 게시글 수정

   서버에서 첨부파일은 UPDATE를 하지 않고 DELETE 후 INSERT로 처리!
   DB에서만 삭제하지 않고 실제 업로드된 경로에서도 삭제가 필요!
   그래서 기존 첨부파일을 모두 가져온 뒤 경로를 알아내고 서버에서 삭제 후
   DB에서 삭제를 진행했다.
